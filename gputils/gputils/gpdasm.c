...
static size_t
_unexpand(char *Dst, size_t Size, const char *Src)
{
  size_t in_read_idx;
...
_write_core_sfr_list(void)
{
  ...
  for (i = state.class->core_sfr_number; i > 0; ++table, --i) {
    ...
    _ux_print(true, "%-7s equ     0x%03x", table->name, table->address);
  }
}
...
static void
_add_constant(lset_section_t *Section, const char *Name, long Start, unsigned int Attr)
{
        ...
        if (state.class == PROC_CLASS_PIC16E) {
          if (!state.show_config) {
            if (b_memory_get(m, i, &byte, NULL, NULL)) {
              ...
              if (state.format) {
                _ux_print(false, "%0*x:  %02x  ", addr_digits, org, (unsigned int)byte);
              } else {
                _ux_print(false, "        ");
              }

              _ux_print(true, "%-*s0x%02x", TABULATOR_SIZE, "db", (unsigned int)byte);
            }
            else {
              last_loc = 0;
            }
          ...
          if (!state.show_config) {
            if (state.class->i_memory_get(m, i, &data, NULL, NULL)) {
              ...
              if (state.format) {
                _ux_print(false, "%0*x:  %0*x  ", addr_digits, org, word_digits, (unsigned int)data);
              } else {
                _ux_print(false, "        ");
              }

              _ux_print(true, "%-*s0x%0*x", TABULATOR_SIZE, "dw", word_digits, (unsigned int)data);
            }
            else {
              last_loc = 0;
            }
